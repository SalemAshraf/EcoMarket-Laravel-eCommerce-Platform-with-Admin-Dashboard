        ...
        <div class="wg-box">
            <form class="form-new-product form-style-1" method="POST" action="{{route('admin.coupon.store')}}" >
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="date" placeholder="Expiry Date" name="expire_date" tabindex="0" value="{{old('expire_date')}}" aria-required="true">
                </fieldset>
                @error("expire_date") <span class="alert alert-danger text-center">{{$message}}</span> @enderror
