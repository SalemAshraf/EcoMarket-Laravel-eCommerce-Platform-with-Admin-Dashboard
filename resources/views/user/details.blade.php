...
@section('content')
    ...
    @if($wishlistItem)
        <form method="POST" id="wishlist-remove-form-{{ $product->id }}"
            action="{{ route('wishlist.remove', ['rowId' => $wishlistItem->rowId]) }}">
            @csrf
            @method("DELETE")
            <a href="javascript:void(0)"
                class="menu-link menu-link_us-s add-to-wishlist filled-heart"
                onclick="document.getElementById('wishlist-remove-form-{{ $product->id }}').submit();">
    ...
    @else
        <form method="POST" action="{{ route('wishlist.add') }}" id="wishlist-add-form-{{ $product->id }}">
            @csrf
            <input type="hidden" name="id" value="{{ $product->id }}" />
            <input type="hidden" name="name" value="{{ $product->name }}" />
            ...
            <a href="javascript:void(0)"
                ...
                onclick="document.getElementById('wishlist-add-form-{{ $product->id }}').submit();">
                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <use href="#icon_heart" />
                </svg>
                                ...
                                <div class="pc__img-wrapper">
                                    <a href="details.php">
                                        ...
                                        @if(count(explode(",", $rproduct->images)) > 0)
                                            <img loading="lazy"
                                                src="{{asset('uploads')}}/{{trim(explode(",", $rproduct->images)[0])}}" width="330"
